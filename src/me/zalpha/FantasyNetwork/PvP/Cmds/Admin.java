...
public class Admin implements Listener, CommandExecutor
{
        ...
        if (label.equalsIgnoreCase("admin")) {
            if (p.hasPermission("fantasy.admin")) {
                if (Manager.Admin.contains(p)) {
                    ...
                    p.getInventory().setArmorContents((ItemStack[])Admin.SalvarArmadura.get(p.getName()));
                    ...
                    Manager.mandarMensagemComPermissao("§7[§a" + p.getName() + " §7saiu do modo ADMIN]", "veravisos", false);
                    ...
                    Manager.mandarMensagemComPermissao("§7[§a" + p.getName() + " §7entrou do modo ADMIN]", "veravisos", false);
                    ...
                    p.getInventory().setItem(1, Manager.criarItem2(Material.DIAMOND_SWORD, "§6\u25ba Players Em PvP", (short)0, 1, new String[] { "§7Clique Para Usar." }));
                    p.getInventory().setItem(2, Manager.criarItem2(Material.BONE, "§b\u25ba Teste De KnockBack (Player)", (short)0, 1, new String[] { "§7Clique Para Usar." }));
                    p.getInventory().setItem(3, Manager.criarItem2(Material.IRON_FENCE, "§b\u25ba Criar Uma Arena (Player)", (short)0, 1, new String[] { "§7Clique Para Usar." }));
                    p.getInventory().setItem(4, Manager.criarItem2(Material.INK_SACK, "§b\u25ba ACABA COM O PC Do Jogador (Player)", (short)15, 1, new String[] { "§7Clique Para Usar." }));
                    p.getInventory().setItem(5, Manager.criarItem2(Material.MUSHROOM_SOUP, "§b\u25ba Checar Auto-Soup (Player)", (short)0, 1, new String[] { "§7Clique Para Usar." }));
                    p.getInventory().setItem(6, Manager.criarItem2(Material.PAPER, "§b\u25ba Teste Force-Field/Kill-Aura (Player)", (short)0, 1, new String[] { "§7Clique Para Usar." }));
                    p.getInventory().setItem(7, Manager.criarItem2(Material.MAGMA_CREAM, "§b\u25ba Troca R\u00e1pida", (short)0, 1, new String[] { "§7Clique Para Usar." }));
                    for (final Player p2 : Bukkit.getOnlinePlayers()) {
        ...
        if (e.getRightClicked() instanceof Player) {
            ...
            if (jogador.getItemInHand().getType() == Material.BONE) {
                if (Admin.Testando.contains(clicado)) {
                    jogador.sendMessage(String.valueOf(Manager.prefix) + " §cDesculpe, Mas Ja Tem Alguem Testando Este Jogador.");
                    return;
                }
                    ...
                    public void run() {
                        if (clicado.isOnline()) {
                            ...
                            if (clicado.getLocation().getY() == localTestado) {
                                jogador.sendMessage(String.valueOf(Manager.prefix) + " §aEste Jogador §fProvavelmente §aPode Estar De Anti-KnockBack.");
                            }
                            else {
                                jogador.sendMessage(String.valueOf(Manager.prefix) + " §cEste Jogador §cN\u00e3o Esta De Anti-KnockBack.");
                            }
                        }
                        else {
                            jogador.sendMessage(String.valueOf(Manager.prefix) + " §aEste Jogador Deslogou Enquanto O Teste Estava Sendo Feito.");
                            Admin.Testando.remove(clicado);
                        }
                    }
                }, 8L);
            ...
            if (jogador.getItemInHand().getType() == Material.IRON_FENCE) {
                ...
                jogador.sendMessage(String.valueOf(Manager.prefix) + " §aVoc\u00ea Criou Uma Arena Para §f" + clicado.getName() + "§a.");
                Manager.mandarMensagemComPermissao("§7[§a" + jogador.getName() + " §7criou uma arena para " + clicado.getName() + "§7]", "veravisos", false);
            }
            if (jogador.getItemInHand().getType() == Material.INK_SACK) {
                clicado.openInventory(Bukkit.createInventory((InventoryHolder)null, 8100));
                jogador.sendMessage(String.valueOf(Manager.prefix) + " §aVoc\u00ea Crashou §f" + clicado.getName() + "§a.");
            }
            if (jogador.getItemInHand().getType().equals((Object)Material.AIR)) {
                jogador.sendMessage(String.valueOf(Manager.prefix) + " §aVoce Esta Vendo O Inventario De §f" + clicado.getName() + "§a.");
                Manager.mandarMensagemComPermissao("§7[§a" + jogador.getName() + " §7esta vendo o inventario de " + clicado.getName() + "§7]", "veravisos", false);
                jogador.openInventory((Inventory)clicado.getInventory());
            }
                    ...
                    public void run() {
                        if (clicado.isOnline()) {
                            ...
                            if (clicado.getInventory().contains(Material.BOWL)) {
                                clicado.getInventory().setContents((ItemStack[])Admin.SalvarInventarioAutoSoup.get(clicado.getName()));
                                clicado.getInventory().setArmorContents((ItemStack[])Admin.SalvarArmaduraAutoSoup.get(clicado.getName()));
                                clicado.updateInventory();
                                jogador.sendMessage(String.valueOf(Manager.prefix) + " §aEste Jogador §fDefinitivamente §aEsta De Auto-Soup.");
                                ...
                                clicado.getInventory().setArmorContents((ItemStack[])Admin.SalvarArmaduraAutoSoup.get(clicado.getName()));
                                clicado.updateInventory();
                                jogador.sendMessage(String.valueOf(Manager.prefix) + " §cEste Jogador §fProvavelmente §cN\u00e3o Pode Estar De Auto-Soup.");
                                clicado.setHealth(20.0);
                            }
                        }
                        else {
            ...
            if (jogador.getItemInHand().getType() == Material.PAPER && jogador.getItemInHand().getItemMeta().getDisplayName().equalsIgnoreCase("§b\u25ba Teste Force-Field/Kill-Aura (Player)")) {
                if (Admin.Testando.contains(clicado)) {
                    jogador.sendMessage(String.valueOf(Manager.prefix) + " §cDesculpe, Mas Ja Tem Alguem Testando Este Jogador.");
                    return;
                }
                ...
                Admin.Testando.add(clicado);
                final Location Local = new Location(clicado.getWorld(), (double)LocalX, (double)LocalY, (double)LocalZ);
                    ...
                    public void run() {
                        if (clicado.isOnline()) {
                            if (Admin.Bateu.contains(clicado)) {
                                ...
                                Admin.Testando.remove(clicado);
                                jogador.sendMessage(String.valueOf(Manager.prefix) + " §aEste Jogador §fDefinitivamente §aEsta De Forcefield.");
                            }
                            else {
                                jogador.sendMessage(String.valueOf(Manager.prefix) + " §cEste Jogador §fProvavelmente §cN\u00e3o Pode Estar De Forcefield.");
                                Admin.Bateu.remove(clicado);
                            }
                        }
                        else {
    ...
    private void modoRapido(final PlayerInteractEvent evento) {
        final Player jogador = evento.getPlayer();
        if (Manager.Admin.contains(jogador)) {
            if (jogador.getItemInHand().getType().equals((Object)Material.MAGMA_CREAM) && !Admin.TrocaRapida.contains(jogador)) {
            ...
            if (jogador.getItemInHand().getType().equals((Object)Material.DIAMOND_SWORD)) {
                if (Manager.Combate.size() >= 1) {
                    final int random = new Random().nextInt(Manager.Combate.size());
                    if (Manager.Combate.get(random).isOnline()) {
                        jogador.teleport((Entity)Manager.Combate.get(random));
                    }
                }
                else {
                    jogador.sendMessage(String.valueOf(Manager.prefix) + " §cDesculpe, Mas N\u00e3o Existe Um Player Em Combate.");
                }
            }
        }
    }
