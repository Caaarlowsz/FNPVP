...
public class Holograms {
	...
	static {
		...
		try {
			...
			Holograms.packetClass = Class
					.forName("net.minecraft.server." + Holograms.version + ".PacketPlayOutSpawnEntityLiving");
			...
			Holograms.destroyPacketClass = Class
					.forName("net.minecraft.server." + Holograms.version + ".PacketPlayOutEntityDestroy");
	...
	public Holograms(final Location loc, final List<String> lines) {
		...
		for (int i = 0; i < lines.size(); ++i) {
			final Object packet = this.getPacket(this.loc.getWorld(), displayLoc.getX(), displayLoc.getY(),
					displayLoc.getZ(), this.lines.get(i).replace("&", "§"));
	...
	private Object getPacket(final World w, final double x, final double y, final double z, final String text) {
		try {
			...
			final Method getHandleMethod = craftWorldObj.getClass().getMethod("getHandle", new Class[0]);
			final Object entityObject = Holograms.armorStandConstructor
					.newInstance(getHandleMethod.invoke(craftWorldObj, new Object[0]));
			...
			final Method setLocation = entityObject.getClass().getMethod("setLocation", Double.TYPE, Double.TYPE,
	...
	private void sendPacket(final Player p, final Object packet) {
		try {
			final Method getHandle = p.getClass().getMethod("getHandle", new Class[0]);
