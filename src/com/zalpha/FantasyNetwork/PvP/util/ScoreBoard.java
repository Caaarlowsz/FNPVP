...
public class ScoreBoard
{
            ...
            for (int i = 1; i < elements.length; ++i) {
                if (elements[i] != null && p.getScoreboard().getObjective(DisplaySlot.SIDEBAR).getScore(elements[i]).getScore() != 16 - i) {
                    ...
                    for (final Object string : p.getScoreboard().getEntries()) {
                        if (p.getScoreboard().getObjective(p.getUniqueId().toString().substring(0, 16)).getScore((String)string).getScore() == 16 - i) {
    ...
    public static boolean unrankedSidebarDisplay(final Collection<Player> players, String[] elements, Scoreboard board) {
        try {
    ...
    public static boolean rankedSidebarDisplay(final Player p, String title, HashMap<String, Integer> elements) {
        try {
            ...
            for (final String string2 : elements.keySet()) {
                if (p.getScoreboard().getObjective(DisplaySlot.SIDEBAR).getScore(string2).getScore() == elements.get(string2)) {
                    continue;
                }
                p.getScoreboard().getObjective(DisplaySlot.SIDEBAR).getScore(string2).setScore((int)elements.get(string2));
            }
    ...
    public static boolean rankedSidebarDisplay(final Collection<Player> players, final String title, final HashMap<String, Integer> elements) {
        for (final Player player : players) {
    ...
    public static boolean rankedSidebarDisplay(final Collection<Player> players, String title, HashMap<String, Integer> elements, Scoreboard board) {
        try {
            title = cutRankedTitle(title);
            elements = cutRanked(elements);
            final String objName = "COLLAB-SB-WINTER";
            ...
            for (final String string2 : elements.keySet()) {
                ...
                board.getObjective(DisplaySlot.SIDEBAR).getScore(string2).setScore((int)elements.get(string2));
            }
