	...
	public SimpleScroller(final String texto, String corPrimaria, String corSecundaria, final int espaco) {
		...
		if (substring + espaco <= texto.length()) {
			while (substring + espaco <= texto.length()) {
				this.frames.add(String.valueOf(corPrimaria) + texto.substring(0, substring += espaco) + corSecundaria
						+ texto.substring(substring));
			}
		}
		substring = 0;
		if (substring + espaco <= texto.length()) {
			while (substring + espaco <= texto.length()) {
				this.frames.add(String.valueOf(corSecundaria) + texto.substring(0, substring += espaco) + corPrimaria
						+ texto.substring(substring));
			}
		}
