...
public abstract class MenuPaged implements Listener
{
    ...
    static {
        MenuPaged.next = Manager.criarItem2(Material.ARROW, "§a\u25ba Pr\u00f3xima pagina", (short)0, 1, new String[] { "§7Clique para ir para o proxima pagina" });
        MenuPaged.previous = Manager.criarItem2(Material.ARROW, "§b\u25ba Pagina anterior", (short)0, 1, new String[] { "§7Clique para ir para o pagina anterior" });
        MenuPaged.kits = Manager.criarItem2(Material.CHEST, "§a\u25ba Kits", (short)0, 1, new String[] { "§7Aqui est\u00e3o os kits que voc\u00ea possui." });
        MenuPaged.warps = Manager.criarItem2(Material.BOOK, "§a\u25ba Warps", (short)0, 1, new String[] { "§7Aqui est\u00e3o as warps." });
    }
    ...
    public MenuPaged(final Player p, final String title, final int size) {
        ...
        Bukkit.getPluginManager().registerEvents((Listener)this, FantasyKits.getPlugin());
    }
    ...
    public boolean addItems(final List<ItemStack> list, final int firstSlot, final int endSlot, final Type t) {
        if (list.isEmpty()) {
            ...
            final Inventory inv = Bukkit.createInventory((InventoryHolder)null, itens.length, this.inventoryTitle.replace("<pag>", String.valueOf(this.inventorys.size() + 1)).replace("<max>", new StringBuilder().append(this.inventorys.size()).toString()).replace("<max>", new StringBuilder().append(this.inventorys.size()).toString()));
            ...
            for (int i = 0; i < list.size(); ++i) {
                ...
                if (this.noUse.containsKey(currentSlot)) {
                    ...
                    if (++currentSlot == endSlot) {
                        final Inventory inv2 = Bukkit.createInventory((InventoryHolder)null, itens2.length, this.inventoryTitle.replace("<pag>", String.valueOf(this.inventorys.size() + 1)).replace("<max>", new StringBuilder().append(this.inventorys.size()).toString()));
                    ...
                    try {
                        ...
                        --i;
                        final Inventory inv3 = Bukkit.createInventory((InventoryHolder)null, itens2.length, this.inventoryTitle.replace("<pag>", String.valueOf(this.inventorys.size() + 1)).replace("<max>", new StringBuilder().append(this.inventorys.size()).toString()));
                    ...
                    if (i + 1 == list.size()) {
                        final Inventory inv4 = Bukkit.createInventory((InventoryHolder)null, itens2.length, this.inventoryTitle.replace("<pag>", String.valueOf(this.inventorys.size() + 1)).replace("<max>", new StringBuilder().append(this.inventorys.size()).toString()));
