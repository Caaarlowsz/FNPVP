...
public class MySQL implements Sql
{
    ...
    public MySQL(final String url_base, final String host, final String database, final String username, final String password) {
        this.host = host;
        this.name = database;
    ...
    private void createTable() {
        try {
            this.connection.createStatement().execute("CREATE TABLE IF NOT EXISTS kitpvp (uuid VARCHAR(36), name VARCHAR(32), stats VARCHAR(255));");
        }
        catch (SQLException e) {
            e.printStackTrace();
        }
