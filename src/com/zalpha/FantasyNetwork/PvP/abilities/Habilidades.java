...
public class Habilidades implements Listener
{
    ...
    static {
        ...
        Habilidades.Particulas = (ArrayList<Player>)Lists.newArrayList();
        Habilidades.Cancel = (ArrayList<Player>)Lists.newArrayList();
    ...
    private void anchor(final EntityDamageByEntityEvent e) {
        if (e.getEntity() instanceof Player && e.getDamager() instanceof Player) {
            ...
            if (Manager.pegarKit(p).equalsIgnoreCase("anchor")) {
                ...
                Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(FantasyKits.getPlugin(), (Runnable)new Runnable() {
                    @Override
                    public void run() {
                        p.setVelocity(new Vector());
                    }
    ...
    public void Madman(final EntityDamageByEntityEvent e) {
        if (e.getEntity() instanceof Player && e.getDamager() instanceof Player) {
            ...
            if (Manager.pegarKit(t) == "Madman" && t.getItemInHand().getType() == Material.STONE_SWORD && new Random().nextInt(100) <= 40) {
    ...
    public void Goku(final PlayerInteractEvent e) {
        ...
        if ((e.getAction() == Action.RIGHT_CLICK_AIR || e.getAction() == Action.RIGHT_CLICK_BLOCK) && Manager.pegarKit(p) == "Goku" && p.getItemInHand().getType() == Material.IRON_BLOCK && (!Manager.Cooldown.containsKey(p.getName()) || Manager.Cooldown.get(p.getName()) <= System.currentTimeMillis())) {
            ...
            h.setVelocity(velo1);
            h.setMetadata("Goku", (MetadataValue)new FixedMetadataValue((Plugin)FantasyKits.getInstace(), (Object)true));
        ...
        else if ((e.getAction() == Action.RIGHT_CLICK_AIR || e.getAction() == Action.RIGHT_CLICK_BLOCK) && Manager.pegarKit(p) == "Goku" && p.getItemInHand().getType() == Material.IRON_BLOCK) {
            p.sendMessage(String.valueOf(Manager.prefix) + "§cAguarde mais §f" + Manager.pegarCooldown(p) + "§c Segundos para usar o GOKU!");
            e.setCancelled(true);
        }
    }
    ...
    private void kangaroo(final PlayerInteractEvent e) {
        final Player jogador = e.getPlayer();
        if ((e.getAction().name().contains("RIGHT") || e.getAction().name().contains("LEFT")) && jogador.getItemInHand().getType() == Material.FIREWORK && Manager.pegarKit(jogador).equalsIgnoreCase("kangaroo")) {
            e.setCancelled(true);
            if (Manager.pegarCombat(jogador)) {
                jogador.sendMessage(String.valueOf(Manager.prefix) + " §cDesculpe, Mais Voce Esta Em Combate Ou Seja Seu Kit Foi Desabilitado Temporariamante.");
    ...
    private void moverkangaroo(final PlayerMoveEvent e) {
        final Player jogador = e.getPlayer();
        if (Manager.pegarKit(jogador).equalsIgnoreCase("kangaroo") && Habilidades.inkangaroo.contains(jogador) && (jogador.getLocation().getBlock().getType() != Material.AIR || jogador.getLocation().getBlock().getRelative(BlockFace.DOWN).getType() != Material.AIR)) {
    ...
    private void kangaroodano(final EntityDamageEvent e) {
        if (e.getEntity() instanceof Player) {
            ...
            if (e.getCause().equals((Object)EntityDamageEvent.DamageCause.FALL) && Manager.pegarKit(jogador).equalsIgnoreCase("kangaroo") && jogador.getInventory().contains(Material.FIREWORK) && e.getDamage() >= 12.0) {
                e.setDamage(12.0);
            }
        }
    }
    ...
    private void switcher(final EntityDamageByEntityEvent e) {
        if (e.getEntity() instanceof Player && e.getDamager() instanceof Snowball) {
            ...
            if (snow.getShooter() instanceof Player && Manager.pegarKit(jogador).equalsIgnoreCase("switcher") && jogadorpuxado != jogador) {
    ...
    private void thor(final PlayerInteractEvent e) {
        final Player jogador = e.getPlayer();
        if (Manager.pegarKit(jogador).equalsIgnoreCase("thor") && e.getAction().name().contains("RIGHT") && jogador.getItemInHand().getType() == Material.STONE_AXE) {
                    ...
                    if (!Habilidades.fighting.containsKey(jogador.getName())) {
                        if (!Habilidades.fighting.containsKey(hitado.getName())) {
                            if (jogador.getLocation().distance(hitado.getLocation()) < 25.0) {
                                Manager.adicionarCooldown(jogador, 7);
                                jogador.teleport((Entity)this.NinjaLocal.get(jogador));
                                jogador.sendMessage(String.valueOf(Manager.prefix) + " §aVoce Teleportou Para §f§l" + hitado.getName() + "§a.");
                                ...
                                jogador.sendMessage(String.valueOf(Manager.prefix) + " §cDesculpe, Mais O Ultimo Jogador Hitado Esta Muito Longe.");
                            }
                        }
                        else {
                            jogador.sendMessage(String.valueOf(Manager.prefix) + " §cDesculpe, Mais O Ultimo Jogador Hitado Esta No Gladiator.");
                        }
                    }
                    else {
                        jogador.sendMessage(String.valueOf(Manager.prefix) + " §cDesculpe, Mais Voce Nao Pode Usar O Ninja No Gladiator.");
                    }
                }
            }
            else {
    ...
    private void gladiator(final PlayerInteractEntityEvent event) {
        ...
        if (p.getItemInHand().getType() == Material.IRON_FENCE && Manager.pegarKit(p).equalsIgnoreCase("gladiator")) {
            ...
            final Location loc2 = new Location(p.getWorld(), (double)(p.getLocation().getBlockX() + 6), (double)(p.getLocation().getBlockY() + 73), (double)(p.getLocation().getBlockZ() + 6));
            final Location loc3 = new Location(p.getWorld(), (double)(p.getLocation().getBlockX() - 5), (double)(p.getLocation().getBlockY() + 73), (double)(p.getLocation().getBlockZ() - 5));
            ...
            if (Habilidades.generateGlass) {
                ...
                for (int bX = -7; bX <= 7; ++bX) {
                    for (int bZ = -7; bZ <= 7; ++bZ) {
                        for (int bY = -1; bY <= 7; ++bY) {
                            ...
                            if (!b.isEmpty()) {
                                event.setCancelled(true);
                                p.sendMessage(String.valueOf(Manager.prefix) + " §cVoce Nao Pode Gerar Um Gladiator Aqui.");
                                return;
                            }
                            if (bY == 7) {
                ...
                p.sendMessage(String.valueOf(Manager.prefix) + " §aVoce Desafiou §f" + r.getName() + " §aPara Um Batalha.");
                p.sendMessage(String.valueOf(Manager.prefix) + " §aCaso N\u00e3o Tenha Um Vencedor Definido Apos §f4 Minutos §aVoce Voltar\u00e1 Ao Seu Lugar De Origem.");
                r.sendMessage(String.valueOf(Manager.prefix) + " §aVoce Foi Desafiado Por §f" + p.getName() + " §aPara Um Batalha.");
                r.sendMessage(String.valueOf(Manager.prefix) + " §aCaso N\u00e3o Tenha Um Vencedor Definido Apos §f4 Minutos §aVoce Voltar\u00e1 Ao Seu Lugar De Origem.");
                Habilidades.fighting.put(p.getName(), r.getName());
                Habilidades.fighting.put(r.getName(), p.getName());
                Habilidades.id2 = Bukkit.getScheduler().scheduleSyncDelayedTask(FantasyKits.getPlugin(), (Runnable)new Runnable() {
                    @Override
                    public void run() {
                        if (Habilidades.fighting.containsKey(p.getName()) && Habilidades.fighting.get(p.getName()).equalsIgnoreCase(r.getName()) && Habilidades.fighting.containsKey(r.getName()) && Habilidades.fighting.get(r.getName()).equalsIgnoreCase(p.getName())) {
                ...
                Habilidades.id1 = Bukkit.getScheduler().scheduleSyncDelayedTask(FantasyKits.getPlugin(), (Runnable)new Runnable() {
                    @Override
                    public void run() {
                        if (Habilidades.fighting.containsKey(p.getName()) && Habilidades.fighting.get(p.getName()).equalsIgnoreCase(r.getName()) && Habilidades.fighting.containsKey(r.getName()) && Habilidades.fighting.get(r.getName()).equalsIgnoreCase(p.getName())) {
                            ...
                            p.sendMessage(String.valueOf(Manager.prefix) + " §cN\u00e3o Houve Um Vencedor Definido, Voce Voltou Ao Seu Lugar De Origem.");
                            r.sendMessage(String.valueOf(Manager.prefix) + " §cN\u00e3o Houve Um Vencedor Definido, Voce Voltou Ao Seu Lugar De Origem.");
    ...
    private void gladiatornaobater(final PlayerInteractEvent e) {
        if (e.getAction() == Action.LEFT_CLICK_BLOCK && e.getClickedBlock().getType() == Material.GLASS && e.getPlayer().getGameMode() != GameMode.CREATIVE && Habilidades.fighting.containsKey(e.getPlayer().getName())) {
            e.setCancelled(true);
    ...
    private void danomagma(final PlayerMoveEvent e) {
        final Player jogador = e.getPlayer();
        if (Manager.pegarKit(jogador).equalsIgnoreCase("magma") && jogador.getLocation().getBlock().getType().name().contains("WATER")) {
            jogador.damage(2.5);
        }
        if (Manager.pegarKit(jogador).equalsIgnoreCase("poseidon") && jogador.getLocation().getBlock().getType().name().contains("WATER")) {
    ...
    private void phantom(final PlayerInteractEvent e) {
        final Player jogador = e.getPlayer();
        if (Manager.pegarKit(jogador).equalsIgnoreCase("phantom") && e.getAction().name().contains("RIGHT") && jogador.getItemInHand().getType() == Material.FEATHER) {
            if (Manager.naoTemCooldown(jogador)) {
                ...
                for (final Entity pertos : jogador.getNearbyEntities(10.0, 10.0, 10.0)) {
                    if (pertos instanceof Player) {
                        ...
                        jogadores.sendMessage(String.valueOf(Manager.prefix) + " §a§lIsto N\u00e3o E Um Hack E Sim Um Kit.");
                    }
                }
                Habilidades.armaduras.put(jogador.getName(), jogador.getInventory().getArmorContents());
                jogador.getInventory().setHelmet(Manager.setarCor(Material.LEATHER_HELMET, Color.WHITE, "§8Armadura Fantasma"));
                jogador.getInventory().setChestplate(Manager.setarCor(Material.LEATHER_CHESTPLATE, Color.WHITE, "§8Armadura Fantasma"));
                jogador.getInventory().setLeggings(Manager.setarCor(Material.LEATHER_LEGGINGS, Color.WHITE, "§8Armadura Fantasma"));
                jogador.getInventory().setBoots(Manager.setarCor(Material.LEATHER_BOOTS, Color.WHITE, "§8Armadura Fantasma"));
                    ...
                    public void run() {
                        ...
                        if (this.tempophantom > 0) {
                            if (this.tempophantom == 1) {
                                jogador.sendMessage(String.valueOf(Manager.prefix) + " §aVoce Tem §f" + this.tempophantom + " Segundo §aDe V\u00f4o.");
                            }
                            else {
                                jogador.sendMessage(String.valueOf(Manager.prefix) + " §aVoce Tem §f" + this.tempophantom + " Segundos §aDe V\u00f4o.");
                            }
                        }
                        if (this.tempophantom == 0) {
                            jogador.sendMessage(String.valueOf(Manager.prefix) + " §cVoce N\u00e3o Pode Mais Voar.");
                            jogador.setAllowFlight(false);
                            jogador.getInventory().setArmorContents((ItemStack[])Habilidades.armaduras.get(jogador.getName()));
                            jogador.updateInventory();
                            this.cancel();
                        }
    ...
    private void phantomnoclickarmadura(final InventoryClickEvent e) {
        ...
        if (e.getCurrentItem() != null && e.getCurrentItem().getItemMeta() != null && e.getCurrentItem().getItemMeta().getDisplayName() != null) {
            if (e.getCurrentItem().getItemMeta().getDisplayName().equalsIgnoreCase("§8Armadura Fantasma") && Manager.pegarKit(jogador).equalsIgnoreCase("phantom")) {
                e.setCancelled(true);
                jogador.updateInventory();
            }
            if (e.getCurrentItem().getItemMeta().getDisplayName().equalsIgnoreCase("§cDeshfire") && Manager.pegarKit(jogador).equalsIgnoreCase("deshfire")) {
                e.setCancelled(true);
                jogador.updateInventory();
            }
            if (e.getCurrentItem().getItemMeta().getDisplayName().equalsIgnoreCase("§9Sonic") && Manager.pegarKit(jogador).equalsIgnoreCase("sonic")) {
                e.setCancelled(true);
                jogador.updateInventory();
            }
        }
    ...
    private void fireman(final EntityDamageEvent e) {
        if (e.getEntity() instanceof Player) {
            final Player jogador = (Player)e.getEntity();
            if ((Manager.pegarKit(jogador).equalsIgnoreCase("fireman") || Manager.pegarKit(jogador).equalsIgnoreCase("magma")) && (e.getCause().name().contains("FIRE") || e.getCause().name().contains("LAVA"))) {
                e.setCancelled(true);
            }
        }
    }
    ...
    private void deshfire(final PlayerInteractEvent e) {
        final Player jogador = e.getPlayer();
        if (e.getAction().name().contains("RIGHT") && jogador.getItemInHand().getType() == Material.REDSTONE_BLOCK && Manager.pegarKit(jogador).equalsIgnoreCase("deshfire")) {
            e.setCancelled(true);
            jogador.updateInventory();
            if (Manager.naoTemCooldown(jogador)) {
                Habilidades.armaduras.put(jogador.getName(), jogador.getInventory().getArmorContents());
                ...
                jogador.getInventory().setChestplate(Manager.setarCor(Material.LEATHER_CHESTPLATE, Color.RED, "§cDeshfire"));
                jogador.getInventory().setLeggings(Manager.setarCor(Material.LEATHER_LEGGINGS, Color.RED, "§cDeshfire"));
    ...
    private void sonic(final PlayerInteractEvent e) {
        final Player jogador = e.getPlayer();
        if (e.getAction().name().contains("RIGHT") && jogador.getItemInHand().getType() == Material.LAPIS_BLOCK && Manager.pegarKit(jogador).equalsIgnoreCase("sonic")) {
            e.setCancelled(true);
            jogador.updateInventory();
            if (Manager.naoTemCooldown(jogador)) {
                Habilidades.armaduras.put(jogador.getName(), jogador.getInventory().getArmorContents());
                ...
                jogador.getInventory().setChestplate(Manager.setarCor(Material.LEATHER_CHESTPLATE, Color.BLUE, "§9Sonic"));
    ...
    private void moverparticulas(final PlayerMoveEvent e) {
        final Player jogador = e.getPlayer();
        if ((Manager.pegarKit(jogador).equalsIgnoreCase("deshfire") || Manager.pegarKit(jogador).equalsIgnoreCase("sonic")) && Habilidades.Cancel.contains(jogador) && jogador.getLocation().getBlock().getRelative(BlockFace.DOWN).getType() != Material.AIR) {
            Habilidades.Particulas.remove(jogador);
            jogador.getInventory().setArmorContents((ItemStack[])Habilidades.armaduras.get(jogador.getName()));
            jogador.updateInventory();
        }
    ...
    private void specialist(final PlayerInteractEvent e) {
        final Player jogador = e.getPlayer();
        if (e.getAction().name().contains("RIGHT") && Manager.pegarKit(jogador).equalsIgnoreCase("specialist") && jogador.getItemInHand().getType() == Material.BOOK) {
    ...
    private void turtlebater(final EntityDamageByEntityEvent e) {
        if (e.getDamager() instanceof Player && e.getEntity() instanceof Player) {
            ...
            if (Manager.pegarKit(jogador).equalsIgnoreCase("turtle") && jogador.isSneaking()) {
                jogador.sendMessage(String.valueOf(Manager.prefix) + " §cVoc\u00ea N\u00e3o Pode Bater Enquanto Estiver No Shift.");
                e.setCancelled(true);
            }
        }
    }
    ...
    private void stomper(final EntityDamageEvent e) {
        if (e.getEntity() instanceof Player) {
            final Player jogador = (Player)e.getEntity();
            if (e.getCause().equals((Object)EntityDamageEvent.DamageCause.FALL) && Manager.pegarKit(jogador).equalsIgnoreCase("stomper")) {
                if (e.getDamage() <= 4.0) {
                    e.setCancelled(true);
                    return;
                }
                for (final Entity jogadorespertos : jogador.getNearbyEntities(4.0, 2.0, 4.0)) {
                    if (jogadorespertos instanceof Player) {
                        ...
                        if (jogadores.isSneaking()) {
                            jogadores.damage(3.5, (Entity)jogador);
                            jogador.getWorld().playSound(jogador.getLocation(), Sound.ANVIL_LAND, 1.0f, -1.0f);
                        }
                        else {
                            jogadores.damage(e.getDamage(), (Entity)jogador);
                            jogador.getWorld().playSound(jogador.getLocation(), Sound.ANVIL_LAND, 1.0f, -1.0f);
                        }
                    }
                }
    ...
    private void headshot(final EntityDamageByEntityEvent e) {
        if (e.getDamager() instanceof Player && e.getEntity() instanceof Player) {
            final Player jogador = (Player)e.getDamager();
            if (Manager.pegarKit(jogador).equalsIgnoreCase("headshot") && String.valueOf(jogador.getEyeLocation().getPitch()).contains("-")) {
                e.setDamage(e.getDamage() + 2.0);
                jogador.getWorld().playSound(jogador.getLocation(), Sound.VILLAGER_HIT, 2.0f, 2.0f);
                jogador.getWorld().playEffect(e.getEntity().getLocation().add(0.0, 1.0, 0.0), Effect.STEP_SOUND, (Object)Material.REDSTONE_BLOCK, 10);
            }
        }
    }
    ...
    private void flash(final PlayerInteractEvent e) {
        final Player jogador = e.getPlayer();
        if (e.getAction().name().contains("RIGHT") && jogador.getItemInHand().getType() == Material.REDSTONE_BLOCK && Manager.pegarKit(jogador).equalsIgnoreCase("flash")) {
            e.setCancelled(true);
            jogador.updateInventory();
            if (Manager.naoTemCooldown(jogador)) {
                Habilidades.armaduras.put(jogador.getName(), jogador.getInventory().getArmorContents());
                ...
                jogador.getInventory().setChestplate(Manager.setarCor(Material.LEATHER_CHESTPLATE, Color.RED, "§4Flash"));
    ...
    private void draigfogo(final PlayerInteractEvent e) {
        final Player jogador = e.getPlayer();
        if (e.getAction().name().contains("LEFT") && jogador.getItemInHand().getType() == Material.BLAZE_POWDER && Manager.pegarKit(jogador).equalsIgnoreCase("draig")) {
            e.setCancelled(true);
            jogador.updateInventory();
            if (Manager.naoTemCooldown(jogador)) {
                Manager.adicionarCooldown(jogador, 20);
                ...
                final Snowball h = (Snowball)jogador.launchProjectile((Class)Snowball.class);
                final Vector velo1 = jogador.getLocation().getDirection().normalize().multiply(10);
                h.setVelocity(velo1);
                h.setMetadata("Draig", (MetadataValue)new FixedMetadataValue(FantasyKits.getPlugin(), (Object)true));
            }
            else {
                Manager.mensagemCooldown(jogador);
            }
        }
        if (e.getAction().name().contains("RIGHT") && jogador.getItemInHand().getType() == Material.BLAZE_POWDER && Manager.pegarKit(jogador).equalsIgnoreCase("draig")) {
            e.setCancelled(true);
            jogador.updateInventory();
            if (Manager.naoTemCooldown(jogador)) {
                Manager.adicionarCooldown(jogador, 20);
                    ...
                    public void run() {
                        ...
                        if (this.tempodraig > 0) {
                            if (this.tempodraig == 1) {
                                jogador.sendMessage(String.valueOf(Manager.prefix) + " §aVoce Tem §f" + this.tempodraig + " Segundo §aDe V\u00f4o.");
                            }
                            else {
                                jogador.sendMessage(String.valueOf(Manager.prefix) + " §aVoce Tem §f" + this.tempodraig + " Segundos §aDe V\u00f4o.");
                            }
                        }
                        if (this.tempodraig == 0) {
                            jogador.sendMessage(String.valueOf(Manager.prefix) + " §cVoce N\u00e3o Pode Mais Voar.");
    ...
    private void thresh(final PlayerInteractEvent e) {
        final Player jogador = e.getPlayer();
        if (e.getAction().name().contains("RIGHT") && jogador.getItemInHand().getType() == Material.LEASH && Manager.pegarKit(jogador).equalsIgnoreCase("thresh")) {
            e.setCancelled(true);
            jogador.updateInventory();
            if (Manager.naoTemCooldown(jogador)) {
                Manager.adicionarCooldown(jogador, 20);
                final Snowball snow = (Snowball)jogador.launchProjectile((Class)Snowball.class);
                ...
                snow.setMetadata("thresh", (MetadataValue)new FixedMetadataValue(FantasyKits.getPlugin(), (Object)true));
            }
            else {
                Manager.mensagemCooldown(jogador);
            }
    ...
    private void bloquear(final PlayerCommandPreprocessEvent event) {
        ...
        if (Habilidades.fighting.containsKey(p.getName()) && event.getMessage().toLowerCase().startsWith("/")) {
            event.setCancelled(true);
            p.sendMessage(String.valueOf(Manager.prefix) + " §cDesculpe, Mais E Este Comando Esta Bloqueado No Gladiator.");
        }
    }
    ...
    public void closeInventoryEvent(final InventoryCloseEvent e) {
        if (e.getInventory() instanceof EnchantingInventory && FantasyKits.getMain().inventories.contains(e.getInventory())) {
    ...
    public void inventoryClickEvent(final InventoryClickEvent e) {
        if (e.getClickedInventory() instanceof EnchantingInventory && FantasyKits.getMain().inventories.contains(e.getInventory()) && e.getSlot() == 1) {
            e.setCancelled(true);
        }
    }
    ...
    public void hadouken(final PlayerInteractEvent e) {
        final Player p = e.getPlayer();
        if ((e.getAction() == Action.RIGHT_CLICK_AIR || e.getAction() == Action.RIGHT_CLICK_BLOCK) && Manager.pegarKit(p).equalsIgnoreCase("Sniper") && p.getItemInHand().getType() == Material.BLAZE_ROD) {
            if (Manager.naoTemCooldown(p)) {
                ...
                final Arrow h = (Arrow)p.launchProjectile((Class)Arrow.class);
                final Vector velo1 = p.getLocation().getDirection().normalize().multiply(30);
                h.setVelocity(velo1);
                p.removePotionEffect(PotionEffectType.SLOW);
                Habilidades.Zoom.remove(p);
                h.setMetadata("sniper", (MetadataValue)new FixedMetadataValue(FantasyKits.getPlugin(), (Object)true));
        ...
        if ((e.getAction() == Action.LEFT_CLICK_AIR || e.getAction() == Action.LEFT_CLICK_BLOCK) && Manager.pegarKit(p).equalsIgnoreCase("Sniper") && p.getItemInHand().getType() == Material.BLAZE_ROD && !Habilidades.Zoom.contains(p)) {
        ...
        else if ((e.getAction() == Action.LEFT_CLICK_AIR || e.getAction() == Action.LEFT_CLICK_BLOCK) && Manager.pegarKit(p).equalsIgnoreCase("Sniper") && p.getItemInHand().getType() == Material.BLAZE_ROD && Habilidades.Zoom.contains(p)) {
            p.removePotionEffect(PotionEffectType.SLOW);
            p.playSound(p.getLocation(), Sound.ENDERDRAGON_WINGS, 2.0f, 2.0f);
            Habilidades.Zoom.remove(p);
        }
